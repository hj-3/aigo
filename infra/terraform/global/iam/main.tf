...
resource "aws_iam_policy" "github_actions_tf_infra" {
  name = "${local.p}-github-actions-tf-infra"
...
resource "aws_iam_role_policy_attachment" "github_actions_tf_infra" {
  role       = aws_iam_role.github_actions_deploy.name
  policy_arn = aws_iam_policy.github_actions_tf_infra.arn
}

resource "aws_iam_policy" "github_actions_tf_data" {
  name = "${local.p}-github-actions-tf-data"
...
resource "aws_iam_role_policy_attachment" "github_actions_tf_data" {
  role       = aws_iam_role.github_actions_deploy.name
  policy_arn = aws_iam_policy.github_actions_tf_data.arn
}

resource "aws_iam_policy" "github_actions_tf_platform" {
  name = "${local.p}-github-actions-tf-platform"
  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      { Sid = "SESFull", Effect = "Allow", Action = ["ses:*"], Resource = "*" },
      { Sid = "SNSFull", Effect = "Allow", Action = ["sns:*"], Resource = "*" },
...
resource "aws_iam_role_policy_attachment" "github_actions_tf_platform" {
  role       = aws_iam_role.github_actions_deploy.name
  policy_arn = aws_iam_policy.github_actions_tf_platform.arn
}
